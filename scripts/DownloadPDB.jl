...
function parse_commandline()
    ...
    s = ArgParseSettings(
        description = "Download gzipped files from PDB.",
...
function _file_names(args)
    ...
    elseif list !== nothing && file === nothing
        ...
        throw(
            ErrorException(
                "You must use --code or --list and the filename; --help for more information.",
            ),
        )
    end
end
    ...
    try
        ...
        if format == "PDBML" || format == "PDBFile"
            MIToS.PDB.downloadpdb(
                input,
                format = (format == "PDBML" ? MIToS.PDB.PDBML : MIToS.PDB.PDBFile),
            )
        else
