...
Args = parse_commandline(
    ...
    ["--lambda", "-L"],
    Dict(:help => "Low count value", :arg_type => Float64, :default => 0.05),
    ["--clustering", "-c"],
    ...
    ["--apc", "-a"],
    Dict(:help => "Use APC correction (MIp)", :action => :store_false),
    ["--samples", "-s"],
    Dict(:help => "Number of samples for Z-score", :arg_type => Int, :default => 100),
    ["--usegap", "-G"],
    Dict(:help => "Use gaps on statistics", :action => :store_true),
    ["--fixedgaps", "-F"],
    Dict(:help => "Fix gaps positions for the random samples", :action => :store_false),
    # Keywords...
    description = """
...
@everywhere begin
    ...
    function script(
        ...
        table =
            hcat(to_table(zscore, diagonal = false), to_table(mip, diagonal = false)[:, 3])
